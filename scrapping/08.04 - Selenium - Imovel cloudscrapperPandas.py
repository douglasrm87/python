...
def mostrarTag (result,texto):
	...
	for tag in result:
		...
		itensVetor.append(tag.get_text())
		if "m²" in tag.get_text():
			itensVetor.append(";")
			matriz.append (itensVetor)
			itensVetor = []
		cont = cont + 1
	return matriz
...
def urlGenerica ():
	...
	# print (itens.text)
	matrizPandas = mostrarTag (itens,"Atributos mesclados")
	
	dataFramePandas = pd.DataFrame(matrizPandas)
